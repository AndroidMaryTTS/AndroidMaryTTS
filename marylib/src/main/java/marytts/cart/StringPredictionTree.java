...
public class StringPredictionTree extends CART {
    ...
    public String getMostProbableString(FeatureVector aFV) {
        ...
        if (bestInd >= stringIdDecoding.length) {
            Log.e(Mary.LOG, "looking up most probable string for feature vector");
            Log.e(Mary.LOG, "index bigger than number of targets");
            Log.e(Mary.LOG, "biggest index is " + (stringIdDecoding.length - 1) + "with the symbol" + stringIdDecoding[stringIdDecoding.length - 1]);
        }
