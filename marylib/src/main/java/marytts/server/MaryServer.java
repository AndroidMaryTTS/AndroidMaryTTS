...
public class MaryServer implements Runnable {
    ...
    @Override
    public void run() {
        Log.i(Mary.LOG, "Starting server.");
        try {
            server = new ServerSocket(MaryProperties.needInteger("socket.port"));

            while (true) {
                Log.i(Mary.LOG, "Waiting for client to connect on port " + server.getLocalPort());
...
//            logger = MaryUtils.getLogger("server");
            try {
                ...
                Log.i(Mary.LOG, "Error parsing request:", e);
                if (clientOut == null) {
                    Log.i(Mary.LOG, "Cannot write to client.");
                } else {
        ...
        private void handle() throws Exception {
            ...
            Log.d(Mary.LOG, "read request: `" + line + "'");

            if (line == null) {
                Log.i(Mary.LOG, "Client seems to have disconnected - cannot read.");
                return;
            }
            ...
            if (handleSynthesisRequest(line)) {
                ...
                Log.d(Mary.LOG, new Exception(
                        "Expected either a line" + nl + "MARY IN=<INPUTTYPE> OUT=<OUTPUTTYPE> [AUDIO=<AUDIOTYPE>]" + nl + "or a line containing only a number identifying a request.").toString());
                throw new Exception(
        ...
        private boolean handleInfoRequest(String inputLine) {
            if (inputLine.startsWith("MARY VERSION")) {
                Log.d(Mary.LOG, "InfoRequest " + inputLine);
        ...
        private boolean handleSynthesisRequest(String inputLine) throws Exception {
            ...
            if (style == null) {
                Log.d(Mary.LOG, "No style requested");
            } else {
                Log.d(Mary.LOG, "Style requested: " + style);
            }
            if (effects == null) {
        ...
        private String parseProtocolParameter(String token, String expectedParameterType, String parameterDescription) throws Exception {
            ...
            if (tt.countTokens() != 2 || !tt.nextToken().equals(expectedParameterType)) {
                Log.d(Mary.LOG, new Exception("Expected " + expectedParameterType + "=<" + parameterDescription + ">").toString());
        ...
        private void parseSynthesisLog(String token, StringTokenizer t) throws Exception {
            ...
            Log.i(Mary.LOG, "Connection info: " + log);
        }
        ...
        private boolean voiceGetFullAudioEffect(String inputLine) {
            ...
            AudioEffect effect = AudioEffects.getEffect(effectName);
            if (effect == null) {
                Log.e(Mary.LOG, "Effect name missing in request!");
                return false;
            }
