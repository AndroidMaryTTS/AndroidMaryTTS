...
public class Request {
    ...
    private static void moveBoundariesIntoParagraphs(Document rawmaryxml) {
        ...
        TreeWalker tw = ((DocumentTraversal) rawmaryxml).createTreeWalker
                (rawmaryxml.getDocumentElement(), NodeFilter.SHOW_ELEMENT,
                        new NameNodeFilter(MaryXML.PARAGRAPH, MaryXML.BOUNDARY), true);
    ...
    private static MaryData extractParagraphAsMaryData(MaryData maryxml,
        ...
        Element voice = (Element) DomUtils.getAncestor(paragraph, MaryXML.VOICE);
        if (voice != null) {
     ...
     */
    public void abort() {
        Log.i(Mary.LOG, "Requesting abort.");
        abortRequested = true;
    }
    ...
    public void process() throws Exception {
        ...
        for (int i = 0; i < len && !abortRequested; i++) {
            ...
            if (DomUtils.getPlainTextBelow(currentInputParagraph).trim().equals("")) {
            ...
            if (outputType.isMaryXML()) {
                ...
                DomUtils.replaceElement(currentInputParagraph, outputNodeList);
            }
        }
        long stopTime = System.currentTimeMillis();
        Log.i(Mary.LOG, "Request processed in " + (stopTime - startTime) + " ms.");
        for (MaryModule m : usedModules) {
            Log.i(Mary.LOG, "   " + m.name() + " took " + timingInfo.get(m) + " ms");
        }
    ...
    private MaryData processOrLookupOneChunk(MaryData oneInputData, MaryDataType oneOutputType, String outputParams)
        ...
        if (oneOutputType.isTextType()) {
            try {
                ...
                Log.w(Mary.LOG, "Problem looking up text in cache", e);
            }
            ...
            try {
                ...
                Log.w(Mary.LOG, "Problem looking up audio in cache", e);
            }
        } else {
        ...
        if (oneOutputType.equals(MaryDataType.AUDIO)
            ...
            if (oneOutputType.isTextType()) {
                ...
                Log.d(Mary.LOG, "Don't know how to cache data of type '" + outputtype + "'");
            }
    ...
    private void insertTextIntoCache(MaryCache cache, String inputtype,
        ...
        try {
            ...
            Log.w(Mary.LOG, "Problem inserting text into cache", e);
        }
    }
    ...
    private MaryData processOneChunk(MaryData oneInputData,
        ...
        Log.d(Mary.LOG, "Determining which modules to usedddd");
        ...
        Log.i(Mary.LOG, "Handling request using the following modules:");
        for (MaryModule m : neededModules) {
            Log.i(Mary.LOG, "- " + m.name() + " (" + m.getClass().getName() + ")");
        }
        ...
        for (MaryModule m : neededModules) {
            ...
            if (m.getState() == MaryModule.MODULE_OFFLINE) {
                ...
                Log.i(Mary.LOG, "Starting module " + m.name());
                m.startup();
...
//                Log.d(Mary.LOG,"Handing the following data to the next module:");
//                ByteArrayOutputStream dummy = new ByteArrayOutputStream();
//                currentData.writeTo(dummy);
//                // side effect: writeTo() writes to log if debug
//            }
            Log.i(Mary.LOG, "Next module: " + m.name());
            ...
            if (MaryRuntimeUtils.veryLowMemoryCondition()) {
                Log.i(Mary.LOG, "Very low memory condition detected (only " + MaryUtils.availableMemory() + " bytes left). Triggering garbage collection.");
                Runtime.getRuntime().gc();
                Log.i(Mary.LOG, "After garbage collection: " + MaryUtils.availableMemory() + " bytes available.");
            }
        }
...
//        if (logger.getEffectiveLevel().equals(Level.DEBUG)) {
//                Log.d(Mary.LOG,"Now splitting the following RAWMARYXML data into chunks:");
//                ByteArrayOutputStream dummy = new ByteArrayOutputStream();
//                try {
...
//            }
        ...
        while ((currentNode = tw.nextNode()) != null) {
            ...
            if (!DomUtils.hasAncestor(currentNode, MaryXML.PARAGRAPH)) {
                // Outside paragraphs:
                if (firstNode == null)
                ...
                if (firstNode != null) { // have something to enclose
                    ...
                    Log.d(Mary.LOG, "Found text node below paragraph; enclosing from '"
                            + first + "' to '" + last + "'");
                    DomUtils.encloseNodesWithNewElement(firstNode, lastNode, MaryXML.PARAGRAPH);
                    firstNode = null;
                    lastNode = null;
                }
            }
...
*/
    ...
    public void writeOutputData(OutputStream outputStream) throws Exception {
        ...
        TimerTask timerTask = new TimerTask() {
            @Override
            public void run() {
                Log.w(Mary.LOG, "Timeout occurred while writing output. Forcefully closing output stream.");
                try {
                    os.close();
