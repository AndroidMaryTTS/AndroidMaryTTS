...
//            }
            ...
            Log.w(Mary.LOG, "Couldn't close info socket properly.", e);
            ok = false;
        }
        ...
        try {
            ...
            Log.w(Mary.LOG, "Couldn't close data socket properly.", e);
            ok = false;
        }
        if (ok)
        ...
        if (MaryRuntimeUtils.lowMemoryCondition()) {
            Log.i(Mary.LOG, "Low memory condition detected (only " + MaryUtils.availableMemory() + " bytes left). Triggering garbage collection.");
