    ...
    public void learnMappingFrames(WeightedCodebookFile codebookFile, WeightedCodebookFeatureCollection fcol, BaselineAdaptationSet sourceTrainingSet, BaselineAdaptationSet targetTrainingSet, int[] map) throws IOException {
        ...
        for (i = 0; i < fcol.indexMapFiles.length; i++) {
            ...
            if (imap.files != null && sourceTrainingSet.items.length > i && targetTrainingSet.items.length > i) {
                ...
                if (!bHeaderWritten) {
                    params.codebookHeader.mfccParams.dimension = (srcFeatures.params).dimension;
                    params.codebookHeader.mfccParams.samplingRate = (srcFeatures.params).samplingRate;
                ...
                if (srcFeatures.mfccs != null && srcFeatures.mfccs != null) {
                    for (j = 0; j < imap.files[0].indicesMap.length; j++) //j is the index for labels
                    {
                        if (srcFeatures.mfccs.length > imap.files[0].indicesMap[j][0] && tgtFeatures.mfccs.length > imap.files[0].indicesMap[j][1]) {
                            ...
                            entry = new WeightedCodebookEntry(0, (srcFeatures.params).dimension);
                    ...
                    for (j = 0; j < imap.files[0].indicesMap.length; j++) //j is the index for labels
                    {
                        ...
                        for (k = imap.files[0].indicesMap[j][0]; k <= imap.files[0].indicesMap[j][1]; k++) {
                            if (k >= 0 && k < srcFeatures.mfccs.length) {
                                ...
                                for (n = 0; n < (srcFeatures.params).dimension; n++)
                                ...
                                //Duration
                                index = SignalProcUtils.frameIndex2LabelIndex(k, sourceLabels, (srcFeatures.params).winsize, (srcFeatures.params).skipsize);
                                if (index > 0)
                        ...
                        if (bSourceOK) {
                            ...
                            for (k = imap.files[0].indicesMap[j][2]; k <= imap.files[0].indicesMap[j][3]; k++) {
                                if (k >= 0 && k < tgtFeatures.mfccs.length) {
                                    ...
                                    for (n = 0; n < (tgtFeatures.params).dimension; n++)
                                    ...
                                    //Duration
                                    index = SignalProcUtils.frameIndex2LabelIndex(k, targetLabels, (tgtFeatures.params).winsize, (tgtFeatures.params).skipsize);
                                    if (index > 0)
        ...
        for (i = 0; i < fcol.indexMapFiles.length; i++) {
            ...
            if (sourceTrainingSet.items.length > i) {
                ...
                if (srcFeatures.mfccs != null) {
                    for (j = 0; j < (srcFeatures.params).numfrm; j++) {
                        totalFramesSrc++;
                        bSourceOK = true;
                ...
                if (tgtFeatures.mfccs != null) {
                    for (j = 0; j < (tgtFeatures.params).numfrm; j++) {
                        totalFramesTgt++;
                        bTargetOK = true;
