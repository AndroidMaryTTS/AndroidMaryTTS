...
public class FDPSOLAProcessor extends VocalTractModifier {
     ...
     *
     * @param datagrams     array of Datagram arrays, one element per SelectedUnit
     * @param rightContexts array of Datagrams, one element per SelectedUnit
     * @param audioformat   passed in from {@link BaseUnitConcatenator#load}
     * @param voicings      array of boolean arrays, matching <b>datagrams</b>
     * @param pitchScales   array of double arrays, matching <b>datagrams</b>, pitch modification factors
     * @param timeScales    array of double arrays, matching <b>datagrams</b>, duration modification factors
     * @return modified audio as a DoubleDataSource audio stream
     * @throws IOException if frames cannot be processed
     * @author steiner
     */
    ...
    public DDSAudioInputStream process(double[] x, int[] pitchMarks, AudioFormat audioformat, boolean[] voicings, double[] pitchScales, double[] timeScales) {
        ...
        for (i = 0; i < pitchMarks.length - numPeriods; i++) {
            bLastInputFrame = i == pitchMarks.length - numPeriods - 1;
    ...
    public double[] processFrame(double[] frmIn, boolean isVoiced, double pscale, double tscale, double escale, double vscale, boolean isLastInputFrame, int currentPeriod, int inputFrameSize) throws IOException {
        ...
        if (repeatSkipCount > -1) {
            ...
            if ((isVoiced && pscale != 1.0) || bWarp) {
                if (fftSize < frmSize) {
                    fftSize = (int) Math.pow(2, (Math.ceil(Math.log(frmSize) / Math.log(2.0))));
                    maxFreq = fftSize / 2 + 1;
                }
            ...
            for (j = 1; j <= repeatSkipCount + 1; j++) {
                ...
                bLastFrame = (isLastInputFrame && j == repeatSkipCount + 1);
