...
public class CepstrumLPCAnalyser {
    ...
    public static double[] lpc2lpcc(double[] oneMinusA, double gain, int cepstrumOrder) {
        ...
        for (m = 1; m <= cepstrumOrder; m++) {
            ...
            if (m <= lpcOrder) {
                acc = 0.0;
                for (k = 1; k < m; k++) acc += ((m - k) * oneMinusA[k] * c[m - k]);
                c[m] = acc / (m) + oneMinusA[m];
            }
            /* Coeffs above the LPC order: */
            else {
                acc = 0.0;
                for (k = 1; k <= lpcOrder; k++) acc += ((m - k) * oneMinusA[k] * c[m - k]);
                c[m] = acc / (m);
            }
        }
    ...
    public static double[] lpcc2lpc(double[] c, int lpcOrder) {
        ...
        for (m = 1; m <= lpcOrder; m++) {
            ...
            if (m <= cepstrumOrder) {
                acc = 0.0;
                for (k = 1; k < m; k++) acc += ((k) * a[m - k] * c[k]);
                a[m] = c[m] - acc / (m);
            }
            /* Coeffs above the Cepstrum order: */
            else {
                acc = 0.0;
                for (k = 1; k <= cepstrumOrder; k++) acc += ((k) * a[m - k] * c[k]);
                a[m] = -acc / (m);
            }
        }
