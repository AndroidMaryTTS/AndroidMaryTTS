...
public class HntmAnalyzer {
            ...
            for (i = 0; i < totalFrames; i++) {
                if (maximumFrequencyOfVoicingsInHz[i] > 0.0f && phases[i] != null) {
                    ...
                    for (k = 1; k < phases[i].length - 1; k++) {
                        ...
                        isPrevTrackVoiced = i > 0 && phases[i - 1] != null && phases[i - 1].length > k;
            ...
            for (i = 0; i < totalFrm; i++) {
                ...
                if (analysisParams.harmonicModel == HntmAnalyzerParams.HARMONICS_PLUS_TRANSIENTS_PLUS_NOISE && labels != null) {
                    ...
                    } else //Perhaps end of an existing transient segment
                    {
                        ...
                        {
                            float endTime = Math.min(((pm.pitchMarks[i] + 0.5f * ws) / fs) + analysisParams.overlapBetweenTransientAndNontransientSectionsInSeconds, output.hnmSignal.originalDurationInSeconds);
    ...
    public ComplexNumber[] estimateComplexAmplitudesUncorrelated(double[] frm, double[] wgtSquared, int L, double f0InHz, double samplingRateInHz) {
        ...
        for (k = 1; k <= L; k++) {
            ...
            for (t = 0; t < M; t++) {
                omega = -1.0 * MathUtils.TWOPI * k * f0InHz * ((t + tShift) / samplingRateInHz);
