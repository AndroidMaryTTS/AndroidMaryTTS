    ...
    public NonharmonicSinusoidalSpeechSignal extracSinusoidsPitchSynchronous(double[] x, PitchMarks pm, float numPeriods, float skipSizeInSeconds, float deltaInHz, int spectralEnvelopeType, float[] initialPeakLocationsInHz) {
        ...
        for (i = 0; i < totalFrm; i++) {
            ...
            if (!bFixedSkipRate) {
                ...
                currentTime = (pm.pitchMarks[i] + 0.5f * params.ws) / params.fs;  //Middle of analysis frame
            } else {
            ...
            */
            ...
            if (initialPeakLocationsInHz == null)
                sinSignal.framesSins[i] = analyze_frame(frm, isOutputToTextFile, spectralEnvelopeType, isVoiced, f0, params);
            else
                sinSignal.framesSins[i] = analyze_frame(frm, isOutputToTextFile, spectralEnvelopeType, isVoiced, f0, initialPeakLocationsInHz[initialPeakLocationsInHz.length - 1] + 50.0f, false, params, initialPeakLocationsInHz);
