...
public class HMMDurationF0Modeller extends InternalModule {
    ...
    public void startup() throws Exception {
        ...
        if (targetFeatureLister == null) {
            Log.i(Mary.LOG, "Starting my own TargetFeatureLister");
     ...
     * we should load the HMMs from the en_US-cmu-slt-hsmm */
        ...
        TreeWalker tw = DomUtils.createTreeWalker(doc, doc, MaryXML.PHONE, MaryXML.BOUNDARY);
      ...
      /* Process label file of Mary context features and creates UttModel um */
            ...
            Log.d(Mary.LOG, "No HMM voice called " + hmmVoiceName);
        }
    ...
    private void setSpeechRateSpecifications(NodeList nl, double percentage, double incriment) {
        ...
        NodeIterator nit = DomUtils.createNodeIterator(rootElement, MaryXML.PHONE, MaryXML.BOUNDARY);
        Element nd;
        double duration = 0.0;
    ...
    private void setModifiedContour(NodeList nl, double[] contour) {
        ...
        for (int i = 0; i < nl.getLength(); i++) {
            ...
            while (m.find()) {
                ...
                int placeIndex = (int) Math.floor(((((phoneEndTime - phoneDuration) - fStart) + partPhone) * 100) / duration);
                if (placeIndex >= 100) {
                    placeIndex = 99;
                }
