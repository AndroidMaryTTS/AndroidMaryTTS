...
public class AcousticModeller extends InternalModule {
    ...
    public MaryData process(MaryData d) throws SynthesisException {
        ...
        if (voice == null) {
            Log.d(Mary.LOG, "No voice found for locale; could not process!");
        ...
        if (models == null) {
            ...
            Log.d(Mary.LOG, "No acoustic models defined in " + voice.getName() + "; could not process!");
            output.setDocument(doc);
            return output;
        }
        assert models != null;
     ...
     *
     * @param elements a List of segment Elements
     */
     ...
     * @throws SynthesisException if the Document or some of the relevant Elements cannot be parsed properly
     */
    private Map<String, List<Element>> parseDocument(Document doc) throws SynthesisException {
        ...
        while ((node = treeWalker.nextNode()) != null) {
            ...
            for (segment = DomUtils.getFirstElementByTagName(node, MaryXML.PHONE); segment != null; segment = DomUtils
