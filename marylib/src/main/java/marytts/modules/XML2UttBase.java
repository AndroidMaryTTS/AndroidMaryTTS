...
//            }
        ...
        if (utterances.size() > 0) {
            Utterance firstUtt = utterances.get(0);
            firstUtt.setFirst(true);
            Utterance lastUtt = utterances.get(utterances.size() - 1);
        ...
        while (topElement != null) {
            NodeIterator tokenIt = ((DocumentTraversal) doc).createNodeIterator
                    (topElement, NodeFilter.SHOW_ELEMENT,
                            new NameNodeFilter(MaryXML.MTU, MaryXML.TOKEN, MaryXML.BOUNDARY),
                            false);
...
//                assert tokenItem != null;
                ...
                if (prosody != null && !DomUtils.isAncestor(sentence, prosody)) {
                ...
                if (prosody != previousProsody) { // object equality is ok
                    if (previousProsody != null
                            && (prosody == null || !DomUtils.isAncestor(previousProsody, prosody))) {
...
//                        assert previousToken != null;
                    ...
                    if (prosody != null
                            && (previousProsody == null || !DomUtils.isAncestor(prosody, previousProsody))) {
                ...
                if (element.getTagName().equals(MaryXML.MTU)) {
                    ...
                    while ((element = (Element) tokenIt.nextNode()) != null
                            ...
                            && DomUtils.isAncestor(mtu, element)) {
                        // do nothing, move on
                    }
    ...
    protected String addOneElement(Utterance utterance, Element element,
            ...
            if (createSylStructRelation) {
                ...
                if (element.hasAttribute("duration")) {
                    ...
                    if (dur > 0) {
                        ...
                        Log.d(Mary.LOG, "In voice '" + maryVoice + "', silence symbol is '" + silence + "'");
        ...
        if (phoneList != null) {
            String[] phoneArray = phoneList.toArray(new String[]{});
            ...
            while ((segElement = (Element) segIt.nextNode()) != null) {
                ...
                if (segElement.hasAttribute("d")) {
                    try {
                        ...
                        Log.d(Mary.LOG, "Cannot parse duration attribute `" +
                ...
                if (createTargetRelation && dur != 0 && mbrTargets != null) {
                    ...
                    while (st.hasMoreTokens()) {
                        ...
                        float pos = totalDur - (1 - Float.parseFloat(posString) * 0.01f) * (dur) * 0.001f;
