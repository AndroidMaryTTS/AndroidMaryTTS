    ...
    public MaryData process(MaryData d)
        ...
        while (c != CharacterIterator.DONE) {
            ...
            if (null != tokenisedText.getAnnotation(JTok.CLASS_ANNO)) {
                ...
                Element t = null;
                if (DomUtils.hasAncestor(tn, MaryXML.TOKEN)) {
                    t = (Element) DomUtils.getAncestor(tn, MaryXML.TOKEN);
                ...
                if (currentTextNode != null && currentTextNode != tn &&
                        !DomUtils.hasAncestor(currentTextNode, MaryXML.TOKEN)) {
                ...
                if (null != tokenisedText.getAnnotation(JTok.BORDER_ANNO)) {
                    ...
                    if (firstTokenInSentence != null) {
                        ...
                        if (!DomUtils.hasAncestor(firstTokenInSentence, MaryXML.SENTENCE) &&
                                !DomUtils.hasAncestor(previousToken, MaryXML.SENTENCE)) {
                            Element firstPara = (Element) DomUtils.getAncestor(firstTokenInSentence, MaryXML.PARAGRAPH);
                            Element lastPara = (Element) DomUtils.getAncestor(previousToken, MaryXML.PARAGRAPH);
                    ...
                    if (tokenisedText.getAnnotation(JTok.BORDER_ANNO) ==
                        ...
                        if (firstTokenInParagraph != null) {
                            assert previousToken != null;
                            if (!DomUtils.hasAncestor(firstTokenInParagraph, MaryXML.PARAGRAPH) &&
                                    !DomUtils.hasAncestor(previousToken, MaryXML.PARAGRAPH)) {
        ...
        NodeIterator tIt = DomUtils.createNodeIterator(doc.getDocumentElement(), MaryXML.TOKEN);
        Element t = null;
        while ((t = (Element) tIt.nextNode()) != null) {
            ...
            if (tokenText.length() > TOKEN_MAXLENGTH) {
                ...
                Log.i(Mary.LOG, "Cutting exceedingly long input token (length " + tokenText.length() + " ) to length " + TOKEN_MAXLENGTH + ":\n"
                        + "before: " + tokenText
                        + "\nafter: " + cutTT);
