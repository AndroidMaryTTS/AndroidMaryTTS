...
public class PronunciationModel extends InternalModule {
    ...
    public void startup() throws Exception {
        ...
        if (fdFilename != null) {
            ...
            for (int fileIndex = 0; fileIndex < fileArray.length; fileIndex++) {
                ...
                if (filePatternMatcher.matches()) {
                    ...
                    //Log.d(Mary.LOG,"Read in tree for " + PhoneNameConverter.normForm2phone(phone));
                }
            }
            Log.d(Mary.LOG, "Reading in feature definition and decision trees finished.");
        ...
        Log.d(Mary.LOG, "Building feature computer finished.");
    }
    ...
    public MaryData process(MaryData d)
        ...
        Log.d(Mary.LOG, "Getting xml-data from document finished.");

        TreeWalker tw = DomUtils.createTreeWalker(doc, doc, MaryXML.TOKEN);
        Element t;
        ...
        while ((t = (Element) tw.nextNode()) != null) {
            ...
            TreeWalker sylWalker = DomUtils.createTreeWalker(doc, t, MaryXML.SYLLABLE);
            Element syllable;
            while ((syllable = (Element) sylWalker.nextNode()) != null) {
                ...
                TreeWalker segWalker = DomUtils.createTreeWalker(doc, syllable, MaryXML.PHONE);
                Element seg;
                ...
                for (Element s : originalSegments) {
                    ...
                    if (treeMap.containsKey(phoneString)) {
                        ...
                        StringPredictionTree tree = treeMap.get(phoneString);
                        ...
                        Log.d(Mary.LOG, "didn't find decision tree for phone (" + phoneString + "). Just keeping it.");
                        predicted = new String[]{phoneString};
                    }
                    Log.d(Mary.LOG, "  Predicted phone in sequence of " + predicted.length + " phones.");
                    // deletions:
        ...
        Log.d(Mary.LOG, "Setting the changed xml document finished.");
        return result;
    }
    ...
    private void createSubStructure(Element token, AllophoneSet allophoneSet) {
        ...
        Element prosody = (Element) DomUtils.getAncestor(token, MaryXML.PROSODY);
    ...
    protected void updatePhAttributesFromPhElements(Element token) {
        ...
        StringBuilder tPh = new StringBuilder();
        TreeWalker sylWalker = DomUtils.createTreeWalker(token, MaryXML.SYLLABLE);
        Element syl;
        while ((syl = (Element) sylWalker.nextNode()) != null) {
            ...
            TreeWalker phWalker = DomUtils.createTreeWalker(syl, MaryXML.PHONE);
            Element ph;
