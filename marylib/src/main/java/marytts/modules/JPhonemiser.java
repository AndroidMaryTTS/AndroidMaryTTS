...
public class JPhonemiser extends InternalModule {
    ...
    public JPhonemiser(String componentName,
        ...
        if (userdictFilename != null) {
            userdictFilename = userdictFilename.replace("./", "");

            if (MaryLink.getContext().getAssets().open(userdictFilename) != null) {
                userdict = readLexicon(userdictFilename);
            } else {
                Log.i(Mary.LOG, "User dictionary '" + userdictFilename + "' for locale '" + getLocale() + "' does not exist. Ignoring.");
            }
        }
    ...
    public MaryData process(MaryData d)
        ...
        NodeIterator it = DomUtils.createNodeIterator(doc, doc, MaryXML.TOKEN);
        Element t = null;
    ...
    protected Map<String, List<String>> readLexicon(String lexiconFilename)
        ...
        BufferedReader lexiconFile = new BufferedReader(new InputStreamReader(MaryLink.getContext().getAssets().open(lexiconFilename)));
        while ((line = lexiconFile.readLine()) != null) {
            ...
            try {
                ...
                Log.w(Mary.LOG, "Lexicon '" + lexiconFilename + "': invalid entry for '" + graphStr + "'", re);
            }
