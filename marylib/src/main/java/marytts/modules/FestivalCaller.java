    ...
    private void writeRelationFiles(String festivalUtt)
        ...
        while ((line = buf.readLine()) != null) {
            if (line.startsWith("==") && !line.startsWith("===")) {
                ...
                Log.d(Mary.LOG, "Writing " + relation + " relation:");
                if (pw != null) {
                    pw.close();
                }
...
*/
    ...
    public static class SimpleFestivalClient {
        ...
        public AudioInputStream process(String request)
            ...
            Log.d(Mary.LOG, "Sending request to Festival:\n" + request);
            ...
            Log.d(Mary.LOG, "Trying to read from Festival...");
            while ((c = fromFestival.read()) != -1) {
                Log.d(Mary.LOG, "Read: " + (char) c);
                if (c == 'W') {
                    c = fromFestival.read();
                    if (c == 'V') {
                        c = fromFestival.read();
                        if (c == '\n') {
                            ...
                            if (!timeoutOccurred) {
                                try {
                                    ...
                                    Log.w(Mary.LOG, "Cannot interpret audio data", e);
                                    return null;
                                }
                            }
                            Log.w(Mary.LOG, "Timeout occurred");
