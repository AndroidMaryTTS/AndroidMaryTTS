...
public class MbrolaVoice extends Voice {
    ...
    private void fillSampaMap() {
        ...
        if (sampamapFilename != null) {
            Log.d(Mary.LOG, "For voice " + getName() + ", filling sampa map from file " + sampamapFilename);
            try {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    try {
                        ...
                        Log.w(Mary.LOG, "Ignoring invalid entry in sampa map file " + sampamapFilename);
                    }
                }
            } catch (IOException ioe) {
                Log.w(Mary.LOG, "Cannot open file '" + sampamapFilename + "' referenced in mary config file field voice." + getName() + ".sampamapfile");
            }

        }
        String sampamap = MaryProperties.getProperty("voice." + getName() + ".sampamap");
        if (sampamap != null) {
            Log.d(Mary.LOG, "For voice " + getName() + ", filling sampa map from config file");
            for (StringTokenizer sst = new StringTokenizer(sampamap); sst.hasMoreTokens(); ) {
                try {
                    addSampaMapEntry(sst.nextToken());
                } catch (IllegalArgumentException iae) {
                    Log.w(Mary.LOG, "Ignoring invalid entry in mary config file, field voice." + getName() + ".sampamap");
                }
            }
        }
    }
    ...
    public String voice2sampa(String voicePhoneme) {
        if (voice2sampaMap != null && voice2sampaMap.containsKey(voicePhoneme))
            return voice2sampaMap.get(voicePhoneme);
        else
            return voicePhoneme;
    }
    ...
    public String sampa2voice(String sampaPhoneme) {
        if (sampa2voiceMap != null && sampa2voiceMap.containsKey(sampaPhoneme))
            return sampa2voiceMap.get(sampaPhoneme);
        else
            return sampaPhoneme;
    }
