...
public class Voice {
    ...
    public Voice(String name, Locale locale,
        ...
        try {
            ...
            Log.d(Mary.LOG, "1 Cannot instantiate voice '" + voiceName + "'");
    ...
    public Voice(String name, WaveformSynthesizer synthesizer) throws MaryConfigurationException {
        ...
        if (config == null) {
            Log.d(Mary.LOG, "Trying to load config for voice '" + voiceName + "' but cannot find it.");
        ...
        try {
            ...
            Log.d(Mary.LOG, "2 Cannot instantiate voice '" + voiceName + "' " + n.getMessage());
            throw new MaryConfigurationException("Cannot instantiate voice '" + voiceName + "'", n);
        }
    }
    ...
    private static void checkIfDefaultVoice(Voice voice) {
        ...
        if (currentDefault == null || currentDefault.wantToBeDefault < voice.wantToBeDefault) {
            Log.i(Mary.LOG, "New default voice for locale " + locale + ": " + voice.getName() + " (desire " + voice.wantToBeDefault + ")");
    ...
    public static Voice getVoice(String name) {
        ...
        if (InterpolatingVoice.isInterpolatingVoiceName(name)) {
            ...
            try {
                ...
                Log.w(Mary.LOG, "Could not create Interpolating voice:", e);
                return null;
            }
        }
    ...
    public static Collection<Voice> getAvailableVoices() {
        ...
        Log.d(Mary.LOG, "getAvailableVoices = " + allVoices);
    ...
    public static Collection<Voice> getAvailableVoices(WaveformSynthesizer synth) {
        Log.d(Mary.LOG, "getAvailableVoices =" + synth);
        if (synth == null) {
    ...
    public static Collection<Voice> getAvailableVoices(WaveformSynthesizer synth, Locale locale) {
        Log.d(Mary.LOG, "getAvailableVoices =" + synth + " LOCALE =" + locale);
        ArrayList<Voice> list = new ArrayList<Voice>();
        for (Voice v : allVoices) {
    ...
    public static Voice getVoice(Locale locale, Gender gender) {
        Log.d(Mary.LOG, "getVoice : " + allVoices);
        for (Voice v : allVoices) {
    ...
    public static Voice getDefaultVoice(Locale locale) {
        ...
        if (v == null) Log.w(Mary.LOG, "Could not find default voice for locale " + locale);
        return v;
    }
    ...
    public static Voice getSuitableVoice(MaryData d) {
        ...
        if (guessedVoice != null)
            Log.d(Mary.LOG, "Guessing default voice `" + guessedVoice.getName() + "'");
        else
            Log.d(Mary.LOG, "Couldn't find any voice at all");
    ...
    private static Lexicon getLexicon(String lexiconClass, String lexiconName) {
        ...
        try {
            Log.d(Mary.LOG, "...loading lexicon...");
            if (lexiconName == null) {
                ...
                if (lexiconName.equals("cmudict04")) {
                    ...
                    if (customAddenda != null) {
                        ...
                        Log.d(Mary.LOG, "...loading custom addenda...");
                        lexicon.load();
                        //open addenda file
                        ...
                        while ((line = addendaIn.readLine()) != null) {
                            if (!line.startsWith("#") && !line.equals("")) {
                                ...
                                lexicon.addAddendum(word, null, phones);
                            }
                        }
                    }
                }
            ...
            Log.e(Mary.LOG, "Could not load lexicon " + lexiconClass + "('" + lexiconName + "')", ex);
        }
    ...
    private void init() throws MaryConfigurationException,
        ...
        try {
            ...
            Log.d(Mary.LOG, "No allophone set specified -- neither for voice '" + getName() + "' nor for locale '" + getLocale() + "'");
        ...
        // Log.d(Mary.LOG, "kecdi burdanda .... 1");
        loadAcousticModels(header);

        // Log.d(Mary.LOG, "kecdi burdanda .... 2");
    ...
    private void loadOldStyleProsodyModels(String header) throws MaryConfigurationException {
        ...
        if (durationGraphFile != null) {
            Log.d(Mary.LOG, "...loading duration graph...");
            try {
                durationGraph = (new DirectedGraphReader()).load(durationGraphFile);
            } catch (IOException e) {
                Log.d(Mary.LOG, "Cannot load duration graph file '" + durationGraphFile + "'");
        ...
        if (f0GraphFile != null) {
            Log.d(Mary.LOG, "...loading f0 contour graph...");
            try {
                ...
                Log.d(Mary.LOG, "Cannot load f0 contour graph file '" + f0GraphFile + "'");
    ...
    private void loadAcousticModels(String header) throws MaryConfigurationException, NoSuchPropertyException, IOException {
        ...
        if (acousticModelsString != null) {
            ...
            //Log.d(Mary.LOG, "1 loadAcousticModels   acousticModelsString=" + acousticModelsString);
            ...
            do {
                ...
                //  Log.d(Mary.LOG, "2 loadAcousticModels   modelName=" + modelName);
                ...
                // Log.d(Mary.LOG, "3 loadAcousticModels   modelType=" + modelType+ " header="+header+" " );

                InputStream modelDataStream = null;// not used for hmm models


                String modelAttributeName = MaryProperties.needProperty(header + "." + modelName + ".attribute");
                // Log.d(Mary.LOG, "4 loadAcousticModels   modelAttributeName=" + modelAttributeName);
                ...
                if (possibleModelTypes == null) {
                    Log.d(Mary.LOG, "Cannot handle unknown model type: " + modelType);
                ...
                try {
                    switch (possibleModelTypes) {
                        ...
                        case HMM:
                            ...
                            //   Log.d(Mary.LOG,"loadAcousticModels switch hmm");
                    ...
                    Log.d(Mary.LOG, "Cannot instantiate model '" + modelName + "' of type '" + modelType +
                            "' from '" + MaryProperties.getProperty(header + "." + modelName + ".data") + "'");
    ...
    private void initFeatureProcessorManager() throws MaryConfigurationException {
        ...
        if (featMgrClass != null) {
            try {
                ...
                Log.d(Mary.LOG, "Cannot initialise voice-specific FeatureProcessorManager "
                        + featMgrClass + " from config file");
            ...
            try {
                ...
                Log.d(Mary.LOG, "Cannot initialise voice-specific FeatureProcessorManager: Class "
            ...
            while (st.hasMoreTokens()) {
                ...
                try {
                    ...
                    if (mm == null) {
                        ...
                        Log.w(Mary.LOG, "Module " + moduleInfo + " is not in the standard list of modules -- will start our own, but will not be able to shut it down at the end.");
                        mm = ModuleRegistry.instantiateModule(moduleInfo);
                        mm.startup();
                    }
                    preferredModules.add(mm);
                } catch (Exception e) {
                    Log.w(Mary.LOG, "Cannot initialise preferred module " + moduleInfo + " for voice " + getName() + " -- skipping.", e);
                }
            }
        }
