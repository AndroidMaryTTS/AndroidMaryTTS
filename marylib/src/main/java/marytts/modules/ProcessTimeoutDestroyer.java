...
public class ProcessTimeoutDestroyer extends Thread {
    ...
    private synchronized void doDestroy() {
        ...
        if (process != null) {
            try {
                ...
                //  Log.i(Mary.LOG,"Problems destroying process: ", e);
            }
            process.destroy();
        }
    ...
    public synchronized void doWait(long time) {
        try {
            ...
            Log.w(Mary.LOG, "Wait interrupted: ", e);
        }
    }
    ...
    public void run() {
        while (true) {
            ...
            if (orderingCustomer == null) {
                Log.i(Mary.LOG, "Waiting for timer request.");
            } else {
                Log.i(Mary.LOG, "Received timer request: " +
                        getTimeLimit() + " ms.");
            }
            doWait(getTimeLimit());
            if (shouldExit()) {
                Log.i(Mary.LOG, "Exiting.");
                return;
            }
            if (getCustomer() == null) { // timer was reset before timeout
                Log.i(Mary.LOG, "Normal operation, timer stopped.");
            } else if (getCustomer() == orderingCustomer) {
                Log.i(Mary.LOG, "Timeout occurred. Destroying Process.");
                // OK, the serious case
                doDestroy();
                doResetTimeLimit();
            }
