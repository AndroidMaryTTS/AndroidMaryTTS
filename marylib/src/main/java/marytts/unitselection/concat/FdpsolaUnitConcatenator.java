...
//                tscales[i][j] = 1.2;
            ...
            Log.d(Mary.LOG, "time scaling factor for unit " + unit.getTarget().getName() + " -> " + targetDur / unitDur);
        }
        return tscales;
    }
...
//    }
    ...
    private double[][] getPhoneBasedDurationScales(List<SelectedUnit> units) {
        ...
        try {
            ...
            Log.w(Mary.LOG, "Could not dump tscales to file");
        }
    ...
    private double[][] getRealizedPitchScales(List<Phone> phones) {
        ...
        for (Phone phone : phones) {
            if (phone.getLeftTargetDuration() > 0) {
                ...
                if (clipped) {
                    Log.d(Mary.LOG, "Left F0 factors for phone " + phone + " contained out-of-range values; clipped to [" + minPitchScaleFactor + ", "
                            + maxPitchScaleFactor + "]");
                }
...
//        updateUnitDataDurations(units, datagrams);
     ...
     * data they describe...
     *
     * @param units     whose data should have its durations updated
     * @param datagrams processed array of arrays of Datagrams which had their durations updated in
