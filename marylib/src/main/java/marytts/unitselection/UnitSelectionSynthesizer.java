...
public class UnitSelectionSynthesizer implements WaveformSynthesizer {
    ...
    public void powerOnSelfTest() throws Error {
        try {
            ...
            if (!unitSelVoice.getDomain().equals("general")) {
                Log.i(Mary.LOG, "Cannot perform power-on self test using limited-domain voice '" + unitSelVoice.getName() + "' - skipping.");
                return;
            }
            ...
            if (exampleText != null) {
                ...
                Log.i(Mary.LOG, "No example text -- no power-on self test!");
            }
        } catch (Throwable t) {
            t.printStackTrace();
    ...
    public AudioInputStream synthesize(List<Element> tokensAndBoundaries, Voice voice, String outputParams)
        ...
        Log.d(Mary.LOG, "Selecting units with a " + unitSel.getClass().getName() + " from a " + database.getClass().getName());
        ...
        // Concatenate:
        Log.d(Mary.LOG, "Now creating audio with a " + unitConcatenator.getClass().getName());
        ...
        for (SelectedUnit su : selectedUnits) {
            ...
            if (maryxmlElement != null) {
                ...
                Log.d(Mary.LOG, "Unit " + su.getTarget().getName() + " of length " + unitDurationInMillis + " ms has no maryxml element.");
            }
        }
//        if (logger.getEffectiveLevel().equals(Level.DEBUG)) {
//            try {
